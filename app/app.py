...
with tab_preds:
    ...
    # Try DB predictions first (if you later write predictions to a table)
    preds_df = None
    try:
        preds_df = extract_predictions()
        # extract_predictions may return a DataFrame even if empty
        if preds_df is None or preds_df.empty:
            preds_df = None
    except Exception:
        preds_df = None

    # Fallback: look for predictions_output.csv generated by ML script
    if preds_df is None:
        try:
            preds_file = Path("database") / "predictions_output.csv"
